...
namespace ArRobot {
	...
	void Robot::AddCommand(Command const& newCommand)
	{
		...
		{
			m_LabelMap.try_emplace(newCommand.GetStringData(), m_Commands.size());
		}
		else
		{
	...
	void Robot::Draw(Arge::Renderer& gfx, Arge::Camera const& camera) const
	{
		...
		if (bDebugVisuals)
		{
			...
			case Move:
			{
				...
				auto const screenDelta{
					m_ParentGrid.GridToScreen(currCommand.GetIntDataAt(0), currCommand.GetIntDataAt(1))
				};
